      ...
      <main className="mx-auto max-w-7xl px-5 py-8 md:px-8 md:py-12">
        ...
        <section className="mt-8 grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4" aria-label="Categorias de produtos">
          {categories.map((category, index) => (
            <Link key={category.key} href={{ pathname: "/catalogo", query: { categoria: category.filterCategory, ...(category.filterSubcategory ? { subcategoria: category.filterSubcategory } : {}) } }} className={`group relative min-h-56 overflow-hidden rounded-[1.75rem] border border-brand-border shadow-sm sm:min-h-72 ${index === 6 ? "col-span-2 lg:col-span-1" : ""}`}>
