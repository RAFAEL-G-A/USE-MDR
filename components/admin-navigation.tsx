  ...
  return (
    <nav className="mx-auto max-w-5xl px-3 py-3 md:px-8" aria-label="Áreas administrativas">
      ...
      {items.map((item) => {
        ...
        return (
          <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`flex min-h-16 items-center justify-center gap-2 rounded-2xl border px-2 py-2.5 text-center transition-colors md:px-5 ${active ? "border-brand bg-brand text-white shadow-lg shadow-brand/15" : "border-brand-border bg-white text-brand hover:bg-brand-soft"}`}>
