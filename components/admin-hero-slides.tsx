...
type SlideResponse = { slide?: { slot: number; image_url: string; eyebrow: string; title: string; description: string; fade_enabled: boolean } };

function emptySlides(): AdminHeroSlide[] {
  return Array.from({ length: 4 }, (_, index) => ({ slot: index + 1, imageUrl: null, eyebrow: "", title: "", description: "", fadeEnabled: true }));
}
...
export function AdminHeroSlides() {
  ...
  useEffect(() => {
    let cancelled = false;
    void supabase.from("hero_slides").select("slot, image_url, eyebrow, title, description, fade_enabled").order("slot", { ascending: true }).then(({ data }) => {
      if (cancelled) return;
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error } = await supabase.functions.invoke<SlideResponse>("manage-hero-slide", { body: requestData });
    if (error || !data?.slide) {
      setFeedback({ type: "error", message: await functionErrorMessage(error, "Não foi possível salvar o destaque.") });
      setSaving(false);
      return;
    }
  ...
  async function handleRemove() {
    ...
    if (error) {
      setFeedback({ type: "error", message: await functionErrorMessage(error, "Não foi possível remover o destaque.") });
    } else {
      setSlides((current) => current.map((slide) => slide.slot === activeSlot ? emptySlides()[activeSlot - 1] : slide));
      setEyebrow(""); setTitle(""); setDescription(""); setFadeEnabled(true); setImageFile(null);
  ...
  return (
    <section className="rounded-[2rem] border border-brand-border bg-white p-5 shadow-soft sm:p-8">
      <div>
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">Cadastre até quatro imagens e escolha individualmente se cada slide deve receber o esmaecimento para destacar os textos.</p>
      </div>

      <div className="mt-6 grid grid-cols-4 gap-2" role="tablist" aria-label="Posições do carrossel">
        {slides.map((slide) => (
          <button key={slide.slot} type="button" role="tab" aria-selected={activeSlot === slide.slot} onClick={() => selectSlide(slide.slot)} disabled={loading} className={`rounded-2xl border px-2 py-3 text-xs font-bold transition-colors ${activeSlot === slide.slot ? "border-brand bg-brand text-white" : "border-brand-border bg-brand-soft/40 text-brand"}`}>
            Slide {slide.slot}
      ...
      {loading ? <p className="mt-7 text-sm text-muted">Carregando os destaques...</p> : (
        <form onSubmit={handleSave} className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="space-y-5">
            <HeroField label="Chamada curta" htmlFor="hero-eyebrow"><input id="hero-eyebrow" value={eyebrow} onChange={(event) => setEyebrow(event.target.value)} maxLength={60} className="form-control" placeholder="Ex.: NOVIDADES" /></HeroField>
            <HeroField label="Título principal" htmlFor="hero-title"><textarea id="hero-title" value={title} onChange={(event) => setTitle(event.target.value)} rows={3} maxLength={120} className="form-control resize-y" placeholder="Ex.: Beleza que combina com o seu jeito." /></HeroField>
            <HeroField label="Descrição" htmlFor="hero-description"><textarea id="hero-description" value={description} onChange={(event) => setDescription(event.target.value)} rows={4} maxLength={300} className="form-control resize-y" placeholder="Texto complementar da campanha." /></HeroField>
            <div className="rounded-2xl border border-brand-border bg-brand-soft/30 p-4">
              <label htmlFor="hero-fade" className="flex cursor-pointer items-center gap-3">
                ...
                <span><strong className="block text-sm text-foreground">Esmaecer imagem</strong><span className="mt-1 block text-xs leading-5 text-muted">Aplica o degradê claro no lado do texto somente neste slide.</span></span>
              </label>
            </div>
          </div>
          ...
          <div>
            <HeroField label="Imagem do destaque" htmlFor="hero-image">
              <label htmlFor="hero-image" className="relative mt-2 flex aspect-[4/3] cursor-pointer items-center justify-center overflow-hidden rounded-[1.75rem] border border-dashed border-brand-border bg-brand-soft/50 text-center transition-colors hover:bg-brand-soft">
                {preview ? <><Image src={preview} alt={`Prévia do destaque ${activeSlot}`} fill unoptimized={Boolean(localPreview)} className="object-cover" />{fadeEnabled && <span className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,234,241,0.98)_0%,rgba(255,225,235,0.9)_38%,rgba(255,218,229,0.28)_65%,rgba(255,218,229,0)_100%)]" aria-hidden="true" />}</> : <span className="max-w-52 px-6 text-sm leading-6 text-muted">Toque para escolher a imagem deste slide.</span>}
              </label>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button type="submit" disabled={saving || removing} className="min-h-13 rounded-full bg-brand px-6 text-xs font-extrabold text-white shadow-lg shadow-brand/20 transition-colors hover:bg-brand-strong disabled:cursor-wait disabled:opacity-60">{saving ? "SALVANDO..." : `SALVAR SLIDE ${activeSlot}`}</button>
              {currentSlide.imageUrl && <button type="button" onClick={handleRemove} disabled={saving || removing} className="min-h-13 rounded-full border border-brand-border px-6 text-xs font-extrabold text-brand transition-colors hover:bg-brand-soft disabled:cursor-wait disabled:opacity-60">{removing ? "REMOVENDO..." : "REMOVER DA PÁGINA INICIAL"}</button>}
            </div>
          </div>
        </form>
      )}
