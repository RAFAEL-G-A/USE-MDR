...
function Overview({ metrics, previous, series, corrections }: { metrics: Metrics; previous: Metrics; series: SeriesPoint[]; corrections: SaleCorrectionNotice[] }) {
  return <div className="space-y-6">
    ...
    <div className="grid items-stretch gap-5 xl:grid-cols-2"><PaymentBreakdown metrics={metrics} /><ProductRankingCard title="Mais vendidos" products={metrics.top_selling_products} valueKey="revenue" /></div>
...
function PaymentBreakdown({ metrics }: { metrics: Metrics }) {
  ...
  return <section className="h-full min-w-0 rounded-[2rem] border border-brand-border bg-white p-5 shadow-soft"><h2 className="font-serif text-2xl">Recebimentos</h2>{entries.length === 0 ? <p className="mt-5 text-sm text-muted">Nenhum recebimento no período.</p> : <div className="mt-5 space-y-4">{entries.map(([key, item]) => <div key={key}><div className="flex flex-wrap justify-between gap-2 text-xs"><strong>{labels[key] ?? key}</strong><span>{money(Number(item.amount))} · {item.sales} venda(s)</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-brand-soft"><div className="h-full rounded-full bg-brand" style={{ width: `${Number(item.amount) / maximum * 100}%` }} /></div></div>)}</div>}</section>;
}

function ProductRankingCard({ title, products, valueKey }: { title: string; products: ProductRanking[]; valueKey: "revenue" | "profit" }) {
  return <section className="h-full min-w-0 rounded-[2rem] border border-brand-border bg-white p-5 shadow-soft"><h2 className="font-serif text-2xl">{title}</h2>{products.length === 0 ? <p className="mt-5 text-sm text-muted">Ainda não há dados suficientes.</p> : <ol className="mt-4 divide-y divide-brand-border/60">{products.map((product, index) => <li key={product.product_id} className="flex min-w-0 items-center gap-3 py-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-brand-soft text-xs font-extrabold text-brand">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{product.product_name}</p><p className="text-xs text-muted">{product.quantity} unidade(s)</p></div><strong className="shrink-0 text-sm text-brand">{money(Number(product[valueKey] ?? 0))}</strong></li>)}</ol>}</section>;
}
