...
type Sale = {
  id: string; product_name: string; quantity: number; unit_price: number; unit_cost: number;
...
export function AdminFinances() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const dashboard = dashboardResult.data as { metrics?: unknown; previous_metrics?: unknown; series?: SeriesPoint[]; expenses?: Expense[]; range?: Range };
  ...
  async function registerSale(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.functions.invoke("manage-sales", { body: {
      ...
      sold_at: new Date(soldAt).toISOString(), notes: String(data.get("notes") ?? "").trim(),
    } });
    if (error) setFeedback({ type: "error", message: await functionErrorMessage(error, "Não foi possível registrar a venda.") });
    else { setFeedback({ type: "success", message: "Venda registrada e estoque atualizado." }); form.reset(); setSelectedProductId(""); setUnitPrice(""); setSoldAt(localDateTimeValue()); refresh(); }
    setSaving(false);
  }

  async function saleAction(action: "settle" | "void", sale: Sale, method?: PaymentMethod) {
    setWorkingId(sale.id); setFeedback(null);
    const { error } = await supabase.functions.invoke("manage-sales", { body: { action, sale_id: sale.id, payment_method: method ?? sale.payment_method } });
  ...
  async function saveExpense(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.functions.invoke("manage-finances", { body: {
      ...
      category: data.get("category"), status: data.get("status"), occurred_at: new Date(String(data.get("occurred_at"))).toISOString(),
  ...
  return <div className="space-y-7">
    ...
    <section className="rounded-[2rem] border border-brand-border bg-white p-4 shadow-soft sm:p-7">
      ...
      {range && <p className="mt-3 text-xs text-muted">Período comercial: {new Date(`${range.start}T12:00:00`).toLocaleDateString("pt-BR")} a {new Date(`${range.end}T12:00:00`).toLocaleDateString("pt-BR")} · horário de Pernambuco</p>}
    </section>
    ...
    {!loading && section === "overview" && <Overview metrics={metrics} previous={previous} series={series} />}
...
function Overview({ metrics, previous, series }: { metrics: Metrics; previous: Metrics; series: SeriesPoint[] }) {
...
function SaleItem({ sale, busy, onAction }: { sale: Sale; busy: boolean; onAction: (action: "settle" | "void", sale: Sale, method?: PaymentMethod) => void }) {
  ...
  return <article className={`rounded-2xl border p-4 ${sale.voided_at ? "border-slate-200 bg-slate-50 opacity-60" : "border-brand-border"}`}><div className="flex justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-extrabold">{sale.product_name}</p><p className="mt-1 text-xs text-muted">{sale.customer_name || "Cliente não informado"} · {sale.quantity} un.</p><p className="mt-1 text-[0.65rem] text-muted">{new Date(sale.sold_at).toLocaleString("pt-BR")}</p></div><div className="text-right"><strong className="text-brand">{money(sale.total_amount)}</strong><p className="mt-1 text-[0.62rem] text-muted">Lucro bruto {money(sale.gross_profit)}</p></div></div>{!sale.voided_at && <div className="mt-3 flex flex-wrap gap-2 border-t border-brand-border/60 pt-3">{sale.payment_status === "pending" && <><select value={method} onChange={(event) => setMethod(event.target.value as PaymentMethod)} className="min-h-9 rounded-full border border-brand-border px-3 text-xs">{Object.entries(paymentLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button type="button" disabled={busy} onClick={() => onAction("settle", sale, method)} className="rounded-full bg-emerald-600 px-4 text-xs font-bold text-white">Receber</button></>}<button type="button" disabled={busy} onClick={() => onAction("void", sale)} className="ml-auto rounded-full border border-red-200 px-4 text-xs font-bold text-red-600">Cancelar</button></div>}</article>;
}
