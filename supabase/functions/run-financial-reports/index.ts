...
type Metrics = Record<string, unknown> & {
  ...
  store_analytics?: {
    visitors?: number; sessions?: number; whatsapp_clicks?: number; checkout_visitors?: number;
    conversion_rate?: number; cart_items?: number; cart_value?: number;
  };
};
...
function buildEmail(type: ReportType, start: string, end: string, metrics: Metrics) {
  ...
  const analytics = metrics.store_analytics ?? {};
  ...
  const analyticsText = `JORNADA DO SITE ATÉ O WHATSAPP\nVisitantes: ${Number(analytics.visitors) || 0}\nSessões: ${Number(analytics.sessions) || 0}\nForam ao WhatsApp: ${Number(analytics.whatsapp_clicks) || 0}\nConversão: ${(Number(analytics.conversion_rate) || 0).toFixed(1).replace(".", ",")}%\nValor potencial dos carrinhos: ${money(analytics.cart_value)}\nItens levados ao WhatsApp: ${Number(analytics.cart_items) || 0}`;
  const text = `USE MDR — Relatório ${reportLabel(type)}\n${range}\nFaturamento: ${money(metrics.gross_revenue)}\nVendas: ${Number(metrics.orders) || 0}\nDinheiro: ${money(payments.cash?.amount)}\nPix: ${money(payments.pix?.amount)}\nCartões: ${money(card)}\nCustos: ${money(metrics.cost_of_goods)}\nDespesas: ${money(metrics.expenses)}\nLucro bruto: ${money(metrics.gross_profit)}\nLucro líquido: ${money(metrics.net_profit)}\nTicket médio: ${money(metrics.average_ticket)}\n\n${analyticsText}${correctionText ? `\n\nCORREÇÕES DE VENDAS\n${correctionText}` : ""}`;
...
async function processPeriod(client: SupabaseClient, period: Period, settings: Record<string, unknown>, email: { apiKey: string; from: string; to: string }) {
  ...
  try {
    const [closureResult, analyticsResult] = await Promise.all([
      client.rpc("create_financial_closure", { p_period_type: period.type, p_period_start: period.start, p_period_end: period.end, p_generated_by: null }),
      client.rpc("get_store_analytics", { p_period_start: period.start, p_period_end: period.end }),
    ]);
    const { data: closureData, error: closureError } = closureResult;
    if (closureError) throw closureError;
    if (analyticsResult.error) throw analyticsResult.error;
    const closure = Array.isArray(closureData) ? closureData[0] : closureData;
    const analyticsPayload = (analyticsResult.data ?? {}) as { summary?: Metrics["store_analytics"] };
    const metrics = { ...(closure.metrics as Metrics), store_analytics: analyticsPayload.summary ?? {} };
