...
function fileExtension(file: File) {
  const byMimeType: Record<string, string> = { "image/jpeg": "jpg", "image/png": "png", "image/webp": "webp" };
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!contentType.includes("multipart/form-data")) return json(request, { error: "Envie os dados no formato multipart/form-data." }, 415);

    const formData = await request.formData();
    const action = readText(formData, "action") || "save";
    const slot = Number(readText(formData, "slot"));
    if (!Number.isInteger(slot) || slot < 1 || slot > 4) return json(request, { error: "Escolha uma posição de 1 a 4." }, 400);
    ...
    if (action === "remove") {
      const { error: deleteError } = await context.adminClient.from("hero_slides").delete().eq("slot", slot);
    ...
    if (eyebrow.length > 60 || title.length > 120 || description.length > 300) return json(request, { error: "Um dos textos ultrapassou o limite permitido." }, 400);
    ...
    if (image instanceof File && image.size > 0) {
      if (!ACCEPTED_IMAGE_TYPES.has(image.type) || image.size > MAX_IMAGE_SIZE) return json(request, { error: "Envie uma imagem JPG, PNG ou WebP de até 5 MB." }, 400);
      uploadedImagePath = `hero/${crypto.randomUUID()}.${fileExtension(image)}`;
      const { error: uploadError } = await context.adminClient.storage.from("products").upload(uploadedImagePath, image, { cacheControl: "31536000", contentType: image.type, upsert: false });
    ...
    if (!imagePath || !imageUrl) return json(request, { error: "Selecione uma imagem para ativar este destaque." }, 400);

    const { data: slide, error: upsertError } = await context.adminClient
      .from("hero_slides")
      .upsert({ slot, image_url: imageUrl, image_path: imagePath, eyebrow, title, description, fade_enabled: fadeEnabled, updated_at: new Date().toISOString() }, { onConflict: "slot" })
      .select("slot, image_url, eyebrow, title, description, fade_enabled")
      .single();

    if (upsertError) {
      if (uploadedImagePath) await context.adminClient.storage.from("products").remove([uploadedImagePath]);
      throw new Error(`Não foi possível salvar o destaque: ${upsertError.message}`);
    }
    if (uploadedImagePath && existing?.image_path && existing.image_path !== uploadedImagePath) await context.adminClient.storage.from("products").remove([existing.image_path]);
